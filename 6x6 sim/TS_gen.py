...
if __name__=="__main__":
    ...
    poses = generate_particle_states(n, m)
            ...
            for aa_status in aa_statuses:
                for i in range(num_d):
                    for move_action in range(0,4,1):
                        ...
                        if out_of_arena(next_pos[i]):
                            for move_action2 in range(0,4,1):
                                transitions.append((
                                    states.index(get_state_code(pos), aa_status),
                                    spit_action([move_action,move_action2]), states.index(-1), drone_death_reward, 1
                                ))
                                continue
                            # Remaining cases
