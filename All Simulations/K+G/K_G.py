...
if __name__=="__main__":
    ...
    num_actions = len(movement_delta1)
    ...
    actions = generate_particle_actions(num_d,num_actions)
                ...
                for aa_status in aa_statuses:
                    for action in actions:
                        ...
                        for i in range(num_d):
                            if(i==0):
                                next_pos[i] = np.array(next_pos[i]) + np.array(movement_delta1[action[i][0]])
                            if(i==1):
                                next_pos[i] = np.array(next_pos[i]) + np.array(movement_delta2[action[i][0]])
