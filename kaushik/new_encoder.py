        ...
        for y_coord in range(4):
            ...
            for aa_statuses in [(0,1), (1,0), (1,1)]:
                for aa_pos in aa_positions:
                    prob_drone_gets_hit = np.sum(aa_statuses)*aa_hit_prob(drone_pos, aa_pos)
                transitions.append((
                ...
                for aa_statuses in [(0,1), (1,0), (1,1)]:
                    ...
                    if np.linalg.norm(np.array(next_pos) - np.array(aa_positions[0])) == 0:
                        ...
                        transitions.append((
                            ...
                            states.index(get_state_code(next_pos, 0, aa_statuses[1])), aa_kill_reward, 1-prob_drone_gets_hit
                        ))
                        # Reward for killing the aa and negative reward for dying
                        transitions.append((
                            states.index(get_state_code(drone_pos, aa_statuses[0], aa_statuses[1])), move_action,
                    ...
                    # Invading airspace of aa2
                        ...
                        transitions.append((
                            states.index(get_state_code(drone_pos, aa_statuses[0], aa_statuses[1])), move_action,
                            states.index(get_state_code(next_pos, aa_statuses[0], 0)), aa_kill_reward, 1-prob_drone_gets_hit
                        ))
                        # Reward for killing the aa and negative reward for dying
                        transitions.append((
                            states.index(get_state_code(drone_pos, aa_statuses[0], aa_statuses[1])), move_action,
